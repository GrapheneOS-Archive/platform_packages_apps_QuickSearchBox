 ...
 * the global [Intent.ACTION_WEB_SEARCH].
 */
class GoogleSearch : Activity() {
    ...
    private fun createLaunchUriIntentFromSearchIntent(intent: Intent?): Intent? {
        ...
        var source = GOOGLE_SEARCH_SOURCE_UNKNOWN
        ...
        return try {
            val searchUri = (mSearchDomainHelper!!.searchBaseUrl
                .toString() + "&source=android-" + source
    ...
    private fun launchPendingIntent(pending: PendingIntent, fillIn: Intent): Boolean {
        return try {
            ...
            Log.i(
                TAG,
                "Pending intent cancelled: $pending"
            )
            false
        }
    }
    ...
    companion object {
        ...
        fun getLanguage(locale: Locale): String {
            ...
            if (DBG) Log.d(
                TAG,
                "language $language, country $country -> hl=$hl"
            )
            return hl.toString()
        }
